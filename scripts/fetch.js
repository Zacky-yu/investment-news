...
function httpGet(url){
  return new Promise((resolve,reject)=>{
    ...
    const ctrl = setTimeout(()=>reject(new Error("timeout")), to);
    mod.get(url, (res)=>{
      let data = "";
      res.on("data", (chunk) => data += chunk.toString());
      res.on("end", () => { clearTimeout(ctrl); resolve(data); });
    }).on("error", (e) => { clearTimeout(ctrl); reject(e); });
  });
}

async function pull(s){
  try{
    const x = await httpGet(s.url);
    const items = [];
...
(async ()=>{
  ...
  const indData = industries.map(ind => {
    ...
    return {key: ind.key, name: ind.name, accent: ind.accent, total: (src.sources||[]).filter(s => s.hint === ind.key).length, items: uniq.filter(i => srcMap[i.source]).slice(0,30)};
  });
